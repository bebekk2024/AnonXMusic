...
async def init():
    if (
        ...
        LOGGER(__name__).error("Assistant Legendaris client variables not defined, exiting...")
        exit()
    await sudo()
    try:
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("GreatXMusic").info("Stopping Legendaris x Music Bot...")
